...
namespace AppointmentBot.Dialogs
{
    ...
    {
        private const string PromptMsgText = "When would you like to have the appointment?";
        private const string RepromptMsgText = "To make your booking please enter an appointment date including time, Day, Month and Year.";
        ...
        private static Task<bool> DateTimePromptValidator(PromptValidatorContext<IList<DateTimeResolution>> promptContext, CancellationToken cancellationToken)
        {
            if (promptContext.Recognized.Succeeded)
            {
                ...
                var timexDate = promptContext.Recognized.Value[0].Timex.Split('T')[0];
            
                // If this is a definite Date including year, month and day we are good otherwise reprompt.
                // A better solution might be to let the user know what part is actually missing.
                var isDefinite = new TimexProperty(timexDate).Types.Contains(Constants.TimexTypes.Definite);
                var includeTime = new TimexProperty(promptContext.Recognized.Value[0].Timex).Types.Contains(Constants.TimexTypes.Time);
                return Task.FromResult(isDefinite && includeTime);
            }
